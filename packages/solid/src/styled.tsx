...
export function verboseStyled<P, Variants, Sizes>(
  ...
  const NewComp = (
    ...
    const [COLOR_MODE, setCOLOR_MODE] = createSignal(get() as 'light' | 'dark');
    ...
    let sxComponentStyleIds;
    let sxDescendantStyleIds;

    let applySxStyleCSSIds;

    let applySxDescendantStyleCSSIdsWithKey;

    const [applySxStateStyleCSSIds, setApplyStateSxStyleCSSIds] = createSignal(
      []
    );
    const [
    ...
    // SX resolution
    let styleTagId = `style-tag-${Math.random().toString().slice(2, 17)}`;
    ...
    let resolvedStyleProps = generateStylePropsFromCSSIds(
      props,
      styleCSSIds,
      globalStyleMap,
      CONFIG
